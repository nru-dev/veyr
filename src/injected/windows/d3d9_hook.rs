...
use super::{LocalProcessMemory, LocalProcessMemoryError};
...
///
/// Unlike the former vtable replacement, this hooks the explicitly configured
/// method entry itself. It therefore needs neither `GxDevice` nor any device
///-layout offset. The original prologue is copied to a trampoline and is
/// invoked through [`Self::call_original`].
...
impl EndSceneHook {
    ...
    /// # Safety
    ///
    /// `target` must be the current process's live `IDirect3DDevice9::EndScene`
    /// entry. `replacement` must remain valid until this hook is uninstalled,
    /// use the exact ABI, and never unwind a panic into the native client.
...
///
/// Reset is a device-lifecycle event, not a frame event. The original method
/// is called through a trampoline between the runtime's before/after-reset
/// callbacks.
...
impl ResetHook {
    ...
    /// # Safety
    ///
    /// `target` must be the current process's live `IDirect3DDevice9::Reset`
    /// entry. `replacement` must remain valid until this hook is uninstalled,
    /// use the exact ABI, and never unwind a panic into the native client.
    pub unsafe fn install(
        target: RemoteAddress,
    ...
    /// # Safety
    ///
    /// `device` and `parameters` must be the values passed to this hook
    /// invocation.
...
impl DirectHook {
    /// Copies complete non-relative x86 instructions into a trampoline and
...
/// Failure while installing or removing a direct D3D9 method detour.
#[derive(Debug)]
...
impl EndSceneHookError {
    ...
    pub const fn diagnostic_code(&self) -> u32 {
        match self {
            Self::NullTarget => 1,
            Self::ReplacementIsOriginal => 2,
            Self::Read(_) => 3,
            Self::AddressOverflow { .. } => 4,
            Self::UnsupportedInstruction { .. } => 5,
            Self::TrampolineAllocation { .. } => 6,
            Self::RelativeJumpOutOfRange { .. } => 7,
            Self::Protect { .. } => 8,
            Self::RestoreProtection { .. } => 9,
            Self::Write(_) => 10,
            Self::FlushInstructionCache { .. } => 11,
            Self::ThreadSnapshot { .. } => 12,
            Self::ThreadSuspend { .. } => 13,
        }
    }
}
